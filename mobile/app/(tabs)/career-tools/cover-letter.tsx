...
export default function CoverLetterScreen() {
  ...
  const roleFocus = useSharedValue(0);
  const companyFocus = useSharedValue(0);
  const skillsFocus = useSharedValue(0);
  ...
  const AnimatedInput = ({ label, value, onChange, placeholder, multiline, focusAnim }: {
    label: string; value: string; onChange: (t: string) => void; placeholder?: string; multiline?: boolean; focusAnim: any;
  }) => {
    const animStyle = useAnimatedStyle(() => ({
      borderColor: focusAnim.value === 1 ? colors.primary : colors.border,
      borderWidth: 1.5,
    }));
    return (
      <View style={styles.fieldWrap}>
        ...
        <Animated.View style={[styles.inputWrap, animStyle]}>
          <TextInput
            ...
            multiline={multiline}
            onFocus={() => { focusAnim.value = withSpring(1, { stiffness: 300, damping: 15 }); }}
            onBlur={() => { focusAnim.value = withSpring(0, { stiffness: 300, damping: 15 }); }}
          />
        </Animated.View>
      </View>
    );
        ...
        >
          ...
          <Animated.View entering={FadeInDown.delay(200).springify().damping(14)} style={styles.formCard}>
            <BlurView intensity={40} tint="light" style={styles.formInner}>
              <AnimatedInput label="Job Role" value={jobRole} onChange={setJobRole} placeholder="e.g. Software Engineer" focusAnim={roleFocus} />
              <AnimatedInput label="Company Name" value={company} onChange={setCompany} placeholder="e.g. Google" focusAnim={companyFocus} />
              <AnimatedInput label="Key Skills (comma separated)" value={skills} onChange={setSkills} placeholder="e.g. React, TypeScript, Node.js" multiline focusAnim={skillsFocus} />
...
const styles = StyleSheet.create({
  ...
  inputWrap: { borderRadius: borderRadius.md, backgroundColor: colors.surface, overflow: 'hidden' },
  input: { paddingHorizontal: spacing.md, paddingVertical: spacing.sm + 4, fontSize: 15, color: colors.text, minHeight: 48 },
